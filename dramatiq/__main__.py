...
def parse_arguments():
    ...
    parser.add_argument(
        "--processes", "-p", default=multiprocessing.cpu_count(),
        type=int, help=f"the number of worker processes to run",
    )
    parser.add_argument(
        "--threads", "-t", default=8,
        type=int, help="the number of worker threads per process",
    )
