...
class GroupCallbacks(Middleware):
    ...
    def after_process_message(self, broker, message, *, result=None, exception=None):
        ...
        if exception is None:
            completion_callbacks = message.options.get("group_callbacks")
            if completion_callbacks is None:
                return
            for uuid, callbacks in completion_callbacks.items():
                barrier = Barrier(self.rate_limiter_backend, uuid, ttl=GROUP_CALLBACK_BARRIER_TTL)
                if barrier.wait(block=False):
                    for message in callbacks.values():
