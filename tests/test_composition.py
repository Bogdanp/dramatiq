...
def test_messages_can_be_piped(stub_broker):
    ...
    assert pipe._messages[0][0].options["pipe_targets"][0] == pipe._messages[1][0].asdict()
    assert pipe._messages[1][0].options["pipe_targets"][0] == pipe._messages[2][0].asdict()
    # assert "pipe_target" not in pipe._messages[2].options
...
def test_pipelines_flatten_child_pipelines(stub_broker):
    ...
    assert pipe._messages[3][0].args == (5,)


def test_pipe_ignore_applies_to_receiving_message(stub_broker, stub_worker, stub_result_backend):
...
def test_pipeline_results_can_be_retrieved(stub_broker, stub_worker, stub_result_backend):
    # Given a result backend
    # And a broker with the results middleware
    stub_broker.add_middleware(Results(backend=stub_result_backend))
    count = 0
...
def test_pipeline_results_respect_timeouts(stub_broker, stub_worker, stub_result_backend):
    # Given a result backend
    # And a broker with the results middleware
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_pipelines_expose_completion_stats(stub_broker, stub_worker, stub_result_backend):
    # Given a result backend
    # And a broker with the results middleware
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_pipelines_can_be_incomplete(stub_broker, stub_result_backend):
...
def test_groups_execute_jobs_in_parallel(stub_broker, stub_worker, stub_result_backend):
...
def test_groups_execute_inner_groups(stub_broker, stub_worker, stub_result_backend):
    # Given that I have a result backend
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_groups_can_time_out(stub_broker, stub_worker, stub_result_backend):
    # Given that I have a result backend
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_groups_expose_completion_stats(stub_broker, stub_worker, stub_result_backend):
    # Given that I have a result backend
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_pipeline_respects_own_delay(stub_broker, stub_worker, stub_result_backend):
    # Given a result backend
    # And a broker with the results middleware
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_pipeline_respects_delay_of_first_message(stub_broker, stub_worker, stub_result_backend):
    # Given a result backend
    # And a broker with the results middleware
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_pipeline_respects_delay_of_second_message(stub_broker, stub_worker, stub_result_backend):
    # Given a result backend
    # And a broker with the results middleware
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_pipeline_respects_bigger_of_first_messages_and_pipelines_delay(stub_broker, stub_worker, stub_result_backend):
    # Given a result backend
    # And a broker with the results middleware
    stub_broker.add_middleware(Results(backend=stub_result_backend))
...
def test_groups_of_pipelines_can_have_completion_callbacks(stub_broker, stub_worker, stub_rate_limiter_backend):
    # Given that I have a rate limiter backend
    # And I've added the GroupCallbacks middleware to my broker
    stub_broker.add_middleware(GroupCallbacks(stub_rate_limiter_backend))
